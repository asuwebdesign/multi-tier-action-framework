...
export const ToolStatus: React.FC<ToolStatusProps> = ({ variant, label }) => {
  const renderIcon = () => {
    switch (variant) {
      ...
      case 'success':
        return <RhUiCheckIcon className="tool-status__icon tool-status__icon--success" />;
      case 'error':
        return <RhUiCloseCircleIcon className="tool-status__icon tool-status__icon--error" />;
    }
  };
  ...
  return (
    ...
    >
      <FlexItem alignSelf={{ default: 'alignSelfCenter' }}>{renderIcon()}</FlexItem>
